...
def detect_live(model, score_filter=0.6):
    ...
    while True:
        ...
        print ("labels:",labels,"boxes:",boxes,"scores:",scores)
        # Plot each box with its label and score
        for i in range(boxes.shape[0]):
            if scores[i] < score_filter:
                continue
            if(labels[i] == 'mavi_priz' and mavi_priz == False):
                mavi_priz = True
                box = boxes[i]
            ...
            elif(labels[i] == 'main_switch' and main_switch == False):
                main_switch = True
                box = boxes[i]
                cv2.rectangle(frame, (box[0], box[1]), (box[2], box[3]), (255, 0, 0), 3)
                if labels:
                    cv2.putText(frame, '{}: {}'.format(labels[i], round(scores[i].item(), 2)), (box[0], box[1] - 10),
                                cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 3)
            elif(labels[i] == 'voltage_knob' and voltage_knob == False):
                voltage_knob = True
                box = boxes[i]
                cv2.rectangle(frame, (box[0], box[1]), (box[2], box[3]), (255, 0, 0), 3)
                if labels:
