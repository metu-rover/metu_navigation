        ...
        for pairs in GivenPairs:  # given pairs include all combinations of all points
            ...
            for obstacle in self.ObstaclesList:
                for edge in obstacle.Edges:  # if the pairs collide with any edge these pairs are deleted for path planning
                    ...
                    if MathOperations.doesCollide(self, pairs, edge):
                        can_draw = False
                        break
            if can_draw == True:
                if not Roughness == False:
                    self.Way_List.append((pairs[0], pairs[1], MathOperations.lenghtOfLines(
                    ...
                    self.Way_List.append(
                        (pairs[0], pairs[1], MathOperations.lenghtOfLines(self, pairs)))
        ...
        for obstacle in self.ObstaclesList:
            ...
            if not obstacle.Roughness == "Infinity":
                ...
                for value, coord in enumerate(obstacle.Coords):
                    self.Way_List.append((coord, obstacle.InsideCoords[value], MathOperations.lenghtOfLines(
                        self, [coord, obstacle.InsideCoords[value]], roughness=obstacle.Roughness)))
...
def visualizer(msg, ObstacleList):
    """""""""*************    IDENTIFIED   OBJECTS    *************"""""""""
    ...
    path = Path(Way, Way.Way_List)  # np save is unpack here

    map1 = Map((150, 100), (1920, 1080))
    endPoints = []  # for multi end points we need an array

    startPoint = Points((int(msg.Current.x), int(msg.Current.y)), path.Ways)
    endPoint = Points((int(msg.Destination.x), int(msg.Destination.y)), path.Ways)
    ...
    ## TODO: HOW TO PUBLISH NEXT POSITION
    next_point = Point()
    return next_point
...
if __name__ == "__main__":
    ...
    rospy.Service('point_creator', ServiceExample, visualizer, ObstacleList)
