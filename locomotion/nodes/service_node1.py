...
class Map():
    def __init__(self,mapsize=tuple,screensize=tuple):                     
        ...
        self.pixelsize=round((1*1.0)/multiply,3) #every pixel in the image equals to this length in the world
        ...
        for pairs in GivenPairs: #given pairs include all combinations of all points
            ...
            for obstacle in self.ObstaclesList:
                for edge in obstacle.Edges: #if the pairs collide with any edge these pairs are deleted for path planning
                    if mathobs1.doesCollide(pairs,edge): #else add to possible accesable points
                        can_draw=False
                        break
            if can_draw==True:
                if not Roughness==False:
                    self.Way_List.append((pairs[0],pairs[1],mathobs1.lenghtOfLines(pairs,roughness=Roughness))) 
                    ...
                    self.Way_List.append((pairs[0],pairs[1],mathobs1.lenghtOfLines(pairs)))
        ...
        for obstacle in self.ObstaclesList:
            ...
            if not obstacle.Roughness=="Infinity":
                ...
                for value,coord in enumerate(obstacle.Coords):
                    self.Way_List.append((coord,obstacle.InsideCoords[value],mathobs1.lenghtOfLines([coord,obstacle.InsideCoords[value]],roughness=obstacle.Roughness)))
...
def visualizer(msg, ObstacleList,mathobses1):
    """""""""*************    IDENTIFIED   OBJECTS    *************"""""""""
...
if __name__ == "__main__":
    Obses = []
    mathobses1=MathOperations()
    rospy.init_node('test_service_server', anonymous=True)

    rospy.Service('test_service', NextPoint, visualizer, (Obses,mathobses1))
